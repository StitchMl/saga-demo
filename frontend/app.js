...
orchestratorForm.addEventListener('submit', async e => {
    ...
    orchestratorFb.textContent = 'Starting the saga…';
    ...
    try {
        ...
        orchestratorFb.textContent = `❌ Errore: ${err.message}`;
    }
});
...
choreoForm.addEventListener('submit', async e => {
    ...
    choreoFb.textContent = 'Starting choreography…';
    ...
    try {
        ...
        if (res.status !== 201) {
            choreoFb.textContent = `❌ Order service returned ${res.status}`;
            return;
        }
        choreoFb.textContent = '✔️ OrderCreated sent: now payment and shipping via Event Bus';
    } catch(err) {
